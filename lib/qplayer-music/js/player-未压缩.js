(function ($) {
    ...
    var loadMusic = function (i) {
        // 避免更改歌单列表，出现错误[如果i>playlist.length,该插件会报错item没被定义]
    ...
    $("#QPlayer .ssBtn").on('click', function(){
        ...
        if ($('.ssBtn .adf').hasClass('on') === false) {
            ...
            // 新增的
            $('.ssBtn .adf .icon-back').show().css("display","block");
            $('.ssBtn .adf .icon-music').hide();
        } else {
            mA.css("transform", "translateX(0px)");
            $('.ssBtn .adf').removeClass('on');
            // 新增的
            $('.ssBtn .adf .icon-music').show();
            $('.ssBtn .adf .icon-back').hide();
        }
    });
    ...
    // 配置项
    if (firstTimeShowPlayerAndList && isFirstPlay) {
        // 是否显示音乐播放器和列表
        $('#QPlayer .liebiao').click();
        $('#QPlayer .ssBtn').click();
    }
    if (!firstTimeShowPlayerAndList && firstTimeShowPlayer && isFirstPlay) {
        // 是否显示音乐播放器
        $('#QPlayer .ssBtn').click();
    }
